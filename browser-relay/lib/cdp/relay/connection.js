...
async function getControlPort() {
  ...
  if (Number.isFinite(raw) && raw > 0 && raw <= 65535) {
    // Accio template used 9234 while bundled desktop listens on 9200 (+2 → WS 9202).
    if (raw === 9234) {
      const next = DEFAULT_CONTROL_PORT
      void chrome.storage.local.set({ controlPort: next, relayPort: computeRelayPort(next) })
      log.info('migrated stored controlPort 9234 →', next, '(match desktop Browser Relay)')
      return next
    }
    return raw
  }
